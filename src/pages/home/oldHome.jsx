...
// ── FloatingButton: deferred until browser is idle so it doesn't block LCP ──
const FloatingButton = dynamic(
  () => import("@/components/FloatingButton"),
  { ssr: false, loading: () => null }
);

// ── BannerSlider: dynamic + ssr:false removes Swiper from SSR bundle ──
// The .banner-slider-wrap placeholder in critical.css reserves height → no CLS.
...
// FertilityChatbot: ssr:false + triggered only after scroll/idle — no LCP impact
...
/**
 * LazySection — only renders {children} once the container enters the viewport.
 * 200px rootMargin gives components time to load before they scroll into view.
 * Placeholder div preserves layout height while content is pending.
 */
...
function Home() {
  // FloatingButton: show only after browser is idle (no LCP competition)
  ...
  // FertilityChatbot: mount only after first scroll or 5s
  const [chatReady, setChatReady] = useState(false);
  useEffect(() => {
    const enable = () => setChatReady(true);
    const onScroll = () => { enable(); window.removeEventListener('scroll', onScroll); };
