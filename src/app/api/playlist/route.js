...
function findPlaylistVideoRenderers(node, results = []) {
  ...
  if (node.playlistVideoRenderer) {
    results.push({ type: "playlistVideoRenderer", data: node.playlistVideoRenderer });
  } else if (node.lockupViewModel && node.lockupViewModel.contentType === "LOCKUP_CONTENT_TYPE_VIDEO") {
    results.push({ type: "lockupViewModel", data: node.lockupViewModel });
  }
...
function parsePlaylistVideos(initialData, limit) {
  ...
  const videos = renderers
    .map((item) => {
      if (item.type === "playlistVideoRenderer") {
        ...
        return { id, title, thumbnail, duration, index: null };
      }
      return null;
    })
    .filter(Boolean);
  ...
  for (const v of videos) {
    ...
    // Fill index if it was null (e.g. for lockupViewModel)
    if (v.index === null) {
      v.index = String(currentIndex);
    }
    currentIndex++;
